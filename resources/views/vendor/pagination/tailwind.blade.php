@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between px-4">
           ...
           <div>
    <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md px-8">
        {{-- Previous Page Link --}}
        @if ($paginator->onFirstPage())
                ...
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <span aria-current="page">
                            <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white bg-blue-600 border border-blue-600 leading-5 rounded-md transition-all duration-200 ease-out dark:bg-blue-500 dark:border-blue-500">
                                {{ $page }}
                            </span>
                        </span>
                    @else
