<div>
    ...
    <fieldset @disabled($isReadOnly) class="min-w-0">
        <div class="form-container">
            ...
            <div class="form-container__content">
                ...
                {{-- BOTONES DE DESCARGA --}}
                <div class="flex flex-wrap justify-center md:justify-end items-center mb-4 gap-3">
                    ...
                    <div class="overflow-x-auto max-w-full">
                        <table class="table-fixed w-full min-w-[950px] border-2 border-gray-300 bg-white">
                            ...
                            <tbody x-data x-init="
                                ...
                                <tr>
                                    <td colspan="6"
                                        class="px-2 py-8 text-center text-gray-500 text-sm italic border border-gray-300">
                                ...
                                <tr wire:key="photo-{{ $photo->id }}" data-id="{{ $photo->id }}"
                                    ...
                                    <td class="px-2 py-1 border border-gray-300 text-center">
                                        <div
                                            class="drag-handle flex flex-col items-center justify-center h-full cursor-grab active:cursor-grabbing p-2 hover:bg-gray-100 rounded transition-colors">
                                            <flux:icon name="bars-3" class="w-6 h-6 text-gray-500 mb-1" />
                                            <span class="text-xs font-bold text-gray-700">{{ $photo->sort_order
                                                }}</span>
                                        </div>
                                    </td>

                                    {{-- 2. Vista Previa (CON LOADER CORREGIDO) --}}
                                    <td class="px-2 py-1 border border-gray-300">
                                        <div class="flex items-center justify-center py-2">
                                            <div @if(!$isPdf) wire:click="openPreview({{ $photo->id }})"
                                                wire:loading.class="opacity-50 cursor-wait"
                                                wire:target="openPreview({{ $photo->id }})"
                                                class="w-32 h-32 bg-gray-50 border border-gray-200 rounded flex items-center justify-center overflow-hidden shadow-sm hover:scale-105 hover:shadow-md transition-all cursor-pointer relative"
                                                @else
                                                class="w-32 h-32 bg-gray-50 border border-gray-200 rounded flex items-center justify-center overflow-hidden shadow-sm relative"
                                                @endif>

                                                {{-- LOADER DE LA TABLA: Centrado absoluto con fondo semitransparente
                                                --}}
                                                <div wire:loading.flex wire:target="openPreview({{ $photo->id }})"
                                                    class="absolute inset-0 z-20 flex items-center justify-center bg-white/70 backdrop-blur-[1px] rounded transition-opacity">
                                                    <flux:icon name="arrow-path"
                                                        class="w-8 h-8 animate-spin text-[#3A8B88]" />
                                                </div>
                                    ...
                                    <td class="px-2 py-1 border border-gray-300">
                                        <div class="flex items-center gap-1.5 h-full">
                                            @if($photosData[$photo->id]['category'] === 'Sin clasificar')
                                            ...
                                            <div class="flex-grow">
                                                <flux:select wire:model="photosData.{{ $photo->id }}.category"
                                                    wire:change="updatePhotoField({{ $photo->id }}, 'category')"
                                    ...
                                    <td class="px-2 py-1 border border-gray-300">
                                        <div class="flex items-center h-full">
                                            <flux:input type="text"
                                                wire:model.blur="photosData.{{ $photo->id }}.description"
                                    ...
                                    <td class="px-2 py-1 border border-gray-300">
                                        <div class="flex items-center justify-around w-full">
                                            @if(!$isPdf)
                                            <flux:button wire:click="rotatePhoto({{ $photo->id }})" icon="arrow-path"
                                                size="sm" class="cursor-pointer btn-intermediary" />
                                            @else
                                            ...
                                            <flux:button
                                                onclick="confirm('¿Eliminar archivo?') || event.stopImmediatePropagation()"
    ...
    <flux:modal name="preview-modal"
        ...
        La forma más segura de centrar contenido de tamaño variable
        en un contenedor de tamaño fijo.
        --}}
        <div class="grid place-items-center w-full min-h-[90vh] bg-white outline-none">
            <img src="{{ asset('storage/' . $previewPhoto->file_path) }}"
                class="max-h-[85vh] max-w-[90vw] object-contain rounded shadow-lg border border-gray-100"
                style="transform: rotate({{ $previewPhoto->rotation_angle }}deg); transform-origin: center;">
        </div>
        @endif
    </flux:modal>
